// G3-DLL.cpp : Defines the exported functions for the DLL application.
//
...
string doSearchTemplate(string key)
{
	// gọi hàm DLL theo kiểu Run-time
	// Định nghĩa prototype của hàm
	typedef string (*SEARCH_TEMPLATE_PROC)(string); 
	
	HINSTANCE hinstLib; 
	SEARCH_TEMPLATE_PROC ProcAddr; 
	...
	if (hinstLib != NULL) { 
		ProcAddr = (SEARCH_TEMPLATE_PROC) GetProcAddress(hinstLib, "searchTemplate"); 
...
LRESULT CALLBACK KeyHookProc(int nCode, WPARAM wParam, LPARAM lParam) 
{
	...
	if (wParam == WM_KEYDOWN)
	{
		...
		for (int i = 0; i < key.length(); i++)
			key[i] = tolower(key[i]);
		int k = 0;
		content = doSearchTemplate(key.c_str());
		if (keyHook->vkCode == VK_RIGHT && content.length())
		{
